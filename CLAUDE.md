...
**Runtime-written favicons 404 in production (broken-image icons).** Next standalone caches the `public/` file listing at startup (`recursiveReadDir` → `publicFolderItems` Set in `node_modules/next/dist/server/lib/router-utils/filesystem.js`). Incoming requests for `/uploads/...` are checked against this Set; files written *after* server start are never served, even though they exist on disk. PM2 restart "fixes" the previously-broken icons by rescanning, but every newly-added link breaks again until the next restart. `next dev` doesn't reproduce because dev mode falls back to direct fs checks.
- Fix already in repo: `src/app/uploads/icons/[file]/route.ts` is a catch-all route handler that reads the file from disk on each request, bypassing the Set entirely. URLs (`/uploads/icons/xxx.png`) and write path in `src/lib/favicon.ts` are unchanged. The route validates the filename against `^[a-f0-9]{16}\.(png|jpg|jpeg|gif|svg|ico|webp)$` to prevent path traversal, so any future change to the hash/extension format in `favicon.ts` must update this regex too.
- The handler reads from `process.cwd()/public/uploads/icons/`, which matches where `favicon.ts` writes. In production `process.cwd()` is `.next/standalone/` (the standalone `server.js` does `process.chdir(__dirname)`), so the file is read out of `.next/standalone/public/uploads/icons/`. In dev, both sides resolve to the repo root. No symlink needed.

**App Router routes can be silently static-optimized → non-GET methods return 405.** If a `route.ts` has only a `GET` with no dynamic signals (no cookies/headers/searchParams/`auth()` in the GET handler), Next 14 marks the whole route as `○` (Static) in the build output. Other method exports (`POST`/`PUT`/`DELETE`) — especially wrapped in HOFs like `export const PUT = auth(...)` whose dynamism Next can't statically detect — will then return **405 Method Not Allowed** in production. Dev mode never optimizes, so this only reproduces after `npm run build`.
- Symptom: `curl -X PUT http://127.0.0.1:3000/api/foo` returns 405 with `Allow: GET, HEAD`, even though `route.js` contains the PUT function.
- Diagnose: `npm run build 2>&1 | grep "/api/"` — any route marked `○` instead of `ƒ` is the bug.
- Fix: add `export const dynamic = 'force-dynamic';` to the affected `route.ts`. This is what `src/app/api/settings/route.ts` does.
